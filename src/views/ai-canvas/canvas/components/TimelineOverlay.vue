...
export default {
  ...
  computed: {
    ...
    timelineElements() {
      ...
      const elements = []
      const circleSize = 16 // 圆形直径
      const lineWidth = 2 // 线宽
      // 圆心X坐标 = padding/2
      const circleCenterX = this.timelinePadding / 2
      const circleLeft = circleCenterX - circleSize / 2 // 圆形left
      const lineLeft = circleCenterX - lineWidth / 2 // 线条left

      for (let i = 0; i < this.rows.length; i++) {
        const row = this.rows[i]
        const isLast = i === this.rows.length - 1
        const nextRow = this.rows[i + 1] // 下一行
        const headerHeight = this.rowHeight + this.margin[1] // 行头高度（包括下边距）
        ...
        if (nextRow) {
          // 有下一行，计算到下一个圆形的距离
          const nextCircleTop =
            nextRow.y * headerHeight + (headerHeight - circleSize) / 2
